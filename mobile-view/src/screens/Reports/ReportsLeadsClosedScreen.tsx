...
export default function ReportsLeadsClosedScreen({ navigation }: any) {
  ...
  const loadLeads = async () => {
    try {
      setLoading(true);
      const data = await apiService.get<any>('/leads?status=Closed&limit=500');
        ...
        {filteredLeads.length === 0 ? (
          ...
          filteredLeads.map((lead) => (
            <View key={lead._id} style={styles.card}>
              ...
              <Text style={styles.infoLine}>Closed On: {lead.createdAt ? new Date(lead.createdAt).toLocaleDateString('en-IN') : '-'}</Text>
            </View>
          ))
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  badge: { ...typography.label.small, color: colors.success, borderWidth: 1, borderColor: colors.success, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999 },
