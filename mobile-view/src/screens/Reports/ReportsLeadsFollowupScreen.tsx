...
export default function ReportsLeadsFollowupScreen({ navigation }: any) {
  ...
  const loadLeads = async () => {
    try {
      setLoading(true);
      const data = await apiService.get<any>('/leads?limit=500');
      const entries = Array.isArray(data) ? data : data?.data || [];
      const followUps = entries.filter((lead: Lead) => {
        if (lead.status === 'Saved') return true;
        if (lead.follow_up_date) {
          const followDate = new Date(lead.follow_up_date);
          return followDate >= new Date();
        }
        return false;
      });
      setLeads(followUps);
...
<View style={styles.filters}>
        <WebInput
          ...
          placeholder="Search by school, contact, or mobile" value={search}
          ...
          filteredLeads.map((lead) => (
            <View key={lead._id} style={styles.card}>
              <View style={styles.cardHeader}>
                <Text style={styles.leadTitle}>{lead.school_name || 'Lead'}</Text>
                {lead.follow_up_date ? <Text style={styles.badge}>Follow: {lead.follow_up_date.split('T')[0]}</Text> : null}
              </View>
              ...
              {lead.remarks ? <Text style={styles.infoLine}>Note: {lead.remarks}</Text> : null}
            </View>
          ))
        )}
      </ScrollView>
