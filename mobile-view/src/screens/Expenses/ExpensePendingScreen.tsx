...
export default function ExpensePendingScreen({ navigation }: any) {
  ...
  const loadData = async () => {
    try {
      ...
      const data = await apiService.get<Expense[]>(`/expenses/manager-pending${params.toString() ? `?${params.toString()}` : ''}`);
  ...
  const navigateToUpdate = (expense: Expense) => {
    const employeeId = expense.employeeId?._id || expense.trainerId?._id;
    if (employeeId) {
      navigation.navigate('ExpenseManagerUpdate', { employeeId, employeeName: expense.employeeId?.name || expense.trainerId?.name || 'Employee' });
    }
  };
  ...
  return (
    <ScreenShell
      title="Manager Pending Expenses"
        ...
        {expenses.length === 0 ? (
          ...
          expenses.map((expense) => (
            <View key={expense._id} style={styles.card}>
              ...
              {expense.pendingMonth && (
                <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>Pending Month:</Text>
                  <Text style={styles.infoValue}>{expense.pendingMonth}</Text>
                </View>
              )}
              <TouchableOpacity style={styles.manageButton} onPress={() => navigateToUpdate(expense)}>
                <Text style={styles.manageButtonText}>Manage Expense</Text>
              </TouchableOpacity>
            </View>
          ))
        )}
...
const styles = StyleSheet.create({
  ...
  manageButton: { marginTop: 12, borderRadius: 10, paddingVertical: 12, alignItems: 'center', backgroundColor: colors.primary },
  manageButtonText: { ...typography.label.medium, color: colors.textLight, fontWeight: '600' },
});
