      ...
      <ScrollView style={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        {services.length === 0 ? (
          ...
          services.map((service) => (
            <TouchableOpacity key={service._id} style={styles.card} onPress={() => navigation.navigate('ServiceEdit', { id: service._id })} activeOpacity={0.7}>
              <View style={styles.cardHeader}>
                <Text style={styles.schoolName}>{service.schoolName || 'N/A'}</Text>
                <View style={[styles.statusBadge, { backgroundColor: getStatusColor(service.status) + '20' }]}>
                  <Text style={[styles.statusBadgeText, { color: getStatusColor(service.status) }]}>{service.status || 'Scheduled'}</Text>
                </View>
              </View>
              <View style={styles.cardBody}>
                <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>Service Type:</Text>
                  <Text style={styles.infoValue}>{service.serviceType || '-'}</Text>
                </View>
                <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>Date:</Text>
                  <Text style={styles.infoValue}>{formatDate(service.serviceDate)}</Text>
                </View>
                {service.zone && (
                  <View style={styles.infoRow}>
                    <Text style={styles.infoLabel}>Zone:</Text>
                    <Text style={styles.infoValue}>{service.zone}</Text>
                  </View>
                )}
              </View>
            </TouchableOpacity>
