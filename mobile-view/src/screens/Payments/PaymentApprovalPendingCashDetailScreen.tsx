...
export default function PaymentApprovalPendingCashDetailScreen({ navigation, route }: any) {
  ...
  const handleApprove = async (statusToSet?: string) => {
    ...
    try {
      ...
      Alert.alert('Success', `Payment ${status.toLowerCase()} successfully`, [
        { text: 'OK', onPress: () => navigation.navigate('PaymentApprovalPendingCash') },
      ]);
    } catch (error: any) {
        ...
        {payment && (
          <>
            <View style={styles.infoCard}>
              ...
              {payment.location && <Text style={styles.infoText}>Location: {payment.location}</Text>}
            </View>
