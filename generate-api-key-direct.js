...
function findBackendDir() {
  const possiblePaths = [
    ...
    __dirname, // If script is in backend directory
    process.cwd() // If script is in backend directory
  ];
  
  for (const dir of possiblePaths) {
    const modelsPath = path.join(dir, 'models', 'ApiKey.js');
    if (fs.existsSync(modelsPath)) {
      return dir;
    }
  }
  throw new Error('Could not find backend directory. Please run from project root or backend directory.');
}

const backendDir = findBackendDir();

// Add backend directory to module path so we can require models
...
Module.prototype.require = function(...args) {
  try {
    ...
    if (err.code === 'MODULE_NOT_FOUND') {
      ...
      try {
        return require.resolve(args[0], { paths: [backendNodeModules, backendDir] });
      } catch (e) {
        throw err;
      }
    }
...
const ApiKey = require(path.join(backendDir, 'models', 'ApiKey'));
